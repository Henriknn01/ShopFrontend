...
<template>
<div>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex-grow grid grid-cols-1 group-hover:opacity-75 gap-16 content-center my-12">
        <h1 class="text-6xl font-bold mx-12">Blog Posts</h1>
        <div v-for="post in visiblePosts" :key="post.id" class="rounded-lg shadow-lg max-h-fit overflow-hidden mx-12 group-hover:opacity-75"  >
            <a :href="/blog/ + post.id">
            <div class="overflow-hidden group-hover:opacity-75 rounded-lg h-96 bg-cover bg-center" :style="{backgroundImage: `url(${post.imageUrl})`}" >
            </div>
            <div class="m-6">

...
<template>
    <div class="bg-white mx-auto max-w-7xl py-6 px-4 sm:py-12 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <div v-else>
            <div class="mb-8">
                <div class="flex">
                    <h1 class="text-3xl flex-1 font-bold mb-4">Order - {{order.id}}</h1>
                    <button @click="shipOrder(order)" class="p-2.5 text-center hover:text-gray-500 inline-flex gap-4 ease-in-out duration-300 hover:gap-2" v-if="order.status === 'Processing'"><TruckIcon class="h-6 w-6" /> Ship order</button>
                </div>
                <hr>
            </div>
                    ...
                    <div class="bg-gray-50 shadow-xl hover:shadow-2xl cursor-pointer transition-shadow ease-in-out duration-300 p-4 rounded mb-4">
                        <div class="flex inline-flex items-center w-full">
                            ...
                            <div class="flex-1 text-center">
                                <label>SKU</label>
                                <h5 class="px-3 font-semibold">{{item.product.sku}}</h5>
                            </div>
                            <div class="flex-1 text-center">

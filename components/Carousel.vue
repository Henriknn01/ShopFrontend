...
const products = await productList.products
...
const updateVisibleProducts = () => {
    try {
        visibleProducts = products// .slice(pageIndex, pageIndex + props.limit);
    } catch (e) {
        console.warn(e);
    }
};
...
function scrollNext() {
    if (pageIndex + props.limit < products.length) {
        pageIndex += props.limit;
        updateVisibleProducts();
    }
};

function scrollPrev() {
    if (pageIndex - props.limit >= 0) {
        pageIndex -= props.limit;
        updateVisibleProducts();
    }
};
...
<template>
    <div class="py-6 sm:py-12 ">
        ...
        <div v-else>
            ...
            <div class="relative" v-if="products > 0">
